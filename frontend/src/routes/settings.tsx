...
import { ChevronDown, ChevronRight, Pencil, Check, Sun, Moon, MessageSquare, Phone } from "lucide-react";
...
function Toggle({ enabled, onToggle }: { enabled: boolean; onToggle: () => void }) {
  return (
    <button
      ...
      className={`relative w-12 h-7 rounded-full transition-colors flex-shrink-0 ${enabled ? "bg-[var(--color-primary)]" : "bg-[var(--color-border)]"}`}
    >
      <span
        className={`absolute top-1 left-1 w-5 h-5 rounded-full bg-white shadow transition-transform ${enabled ? "translate-x-5" : "translate-x-0"}`}
      />
    </button>
  );
}
...
function AlertToggleCard() {
  const { smsEnabled, callEnabled, setSmsEnabled, setCallEnabled } = useAlerts();
  return (
    <div className="reef-card p-6 space-y-5">
      <h3 className="font-display text-lg">Notification channels</h3>
      <div className="flex items-center gap-4 justify-between">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-[var(--color-accent-soft)] grid place-items-center">
            <MessageSquare size={15} className="text-[var(--color-primary)]" strokeWidth={1.75} />
          </div>
          <div>
            <div className="text-[14px]">SMS alerts</div>
            <div className="text-[12px] text-[var(--color-text-muted)]">Triggered at ≥ 20% bleaching risk</div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-4 justify-between">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-[var(--color-accent-soft)] grid place-items-center">
            <Phone size={15} className="text-[var(--color-primary)]" strokeWidth={1.75} />
          </div>
          <div>
            <div className="text-[14px]">Voice call alerts</div>
            <div className="text-[12px] text-[var(--color-text-muted)]">Triggered at ≥ 50% bleaching risk</div>
          </div>
        </div>
        <Toggle enabled={callEnabled} onToggle={() => setCallEnabled(!callEnabled)} />
      </div>
    </div>
  );
}
...
function ContactCard() {
  ...
  const runTest = async (k: "sms" | "call") => {
    ...
    if (k === "sms") {
      const result = await api.testSms(phone);
      if (result.ok) {
        setTest((s) => ({ ...s, sms: "ok" }));
        toast.success("Test SMS sent");
      } else {
        setTest((s) => ({ ...s, sms: undefined }));
        toast.error(result.error ?? "SMS failed");
      }
    } else {
    ...
    <div className="reef-card p-6">
      ...
      <div className="mt-6 space-y-4">
        ...
        <button
          disabled={!valid}
          onClick={async () => {
            const result = await api.saveContact(phone);
            if (result.ok) toast.success("Contact saved");
